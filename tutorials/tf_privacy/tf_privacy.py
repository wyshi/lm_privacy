...
The command works:
python /home/wyshi/privacy/tutorials/tf_privacy.py  --batch_size 64 --microbatches 64 --model_dir tutorials/data/model/ 2>&1 | tee tutorials/data/model/log
"""
...
def load_data():
  ...
  if not FLAGS.data_dir:
    ...
    train_dataset = tfds.load(name='lm1b',
                              split=tfds.Split.TRAIN,
                              batch_size=NB_TRAIN,
                              shuffle_files=True)
    test_dataset = tfds.load(name='lm1b',
                             split=tfds.Split.TEST,
                             batch_size=10000)
    train_txt = next(iter(tfds.as_numpy(train_dataset)))
    test_txt = next(iter(tfds.as_numpy(test_dataset)))
    # build vocab set
    vocabs = []
    for t in train_txt['text']:
      vocabs.extend(t.split())
    vocab_to_id = {k:i+1 for i, k in enumerate(vocabs)}
    train_data = []
    test_data = []
    for x in train_txt['text']:
      train_data.extend([vocab_to_id[t] for t in x.split()])
    for x in test_txt['text']:
      test_data.extend([vocab_to_id[t] if t in vocab_to_id else 0 for t in x.split()])
    ...
    remap_test = {k: i for i, k in enumerate(keys)}
...
def main(unused_argv):
  ...
  conf = tf.estimator.RunConfig(#train_distribute=distribution, 
