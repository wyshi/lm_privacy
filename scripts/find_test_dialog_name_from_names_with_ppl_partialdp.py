...
df = pd.read_csv('attacks/membership_inference/candidates/dialog-pick-names-partialdp/1000names_with_ppl.csv')
...
with open("attacks/membership_inference/candidates/dialog-pick-names-partialdp/test/test.txt", "w") as fh:
    fh.writelines([t +"\n" for t in name_not_intrain])

with open("attacks/membership_inference/candidates/dialog-pick-names-partialdp/train/train.txt", "w") as fh:
...
with open("attacks/membership_inference/candidates/dialog-pick-names-partialdp-without-ground-truth-help/test/test.txt", "w") as fh:
    fh.writelines([t +"\n" for t in name_not_intrain])

with open("attacks/membership_inference/candidates/dialog-pick-names-partialdp-without-ground-truth-help/train/train.txt", "w") as fh:
    fh.writelines([t +"\n" for t in name_in_train])