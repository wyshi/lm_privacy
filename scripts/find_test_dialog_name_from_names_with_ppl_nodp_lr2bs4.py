...
df = pd.read_csv('attacks/membership_inference/candidates/dialog-pick-name-nodplr2bs4/1000names_with_ppl.csv')
...
with open("attacks/membership_inference/candidates/dialog-pick-name-nodplr2bs4/test/test.txt", "w") as fh:
    fh.writelines([t +"\n" for t in name_not_intrain])

with open("attacks/membership_inference/candidates/dialog-pick-name-nodplr2bs4/train/train.txt", "w") as fh:
...
with open("attacks/membership_inference/candidates/dialog-pick-name-nodplr2bs4-without-ground-truth-help/test/test.txt", "w") as fh:
    fh.writelines([t +"\n" for t in name_not_intrain])

with open("attacks/membership_inference/candidates/dialog-pick-name-nodplr2bs4-without-ground-truth-help/train/train.txt", "w") as fh:
    fh.writelines([t +"\n" for t in name_in_train])