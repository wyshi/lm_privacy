...
class BeliefSlot(object):
    ...
    def get_maxconf_value(self):
        if len(self.value_map) == 0:
            return None
        value_map_items = [(s, v) for v, s in self.value_map.items()]
        value_map_items_cleaned = [item for item in value_map_items if None not in item]
        if len(value_map_items_cleaned) == 0:
            return None
        max_s, max_v = max(value_map_items_cleaned)
        return max_v
